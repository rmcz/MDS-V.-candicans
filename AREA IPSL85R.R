...
##########ACCES45LIMA
#Cargar datos
map <- raster("D:/Maxent/AREA Y LIMA2/Vasconcellea_candicans_IPSL85.asc")
plot(map)
...
levelplot(cat.act, at = breaks, col.regions = cols, main = "       IPSL-CM6A-LR RCP 8.5 (2081 - 2100)",
...
aact #aha
########BARPLOT
bp<- barplot(aact, ylim=c(0,1375612),
             #main = "Area (km^2)",
             xlab = "Idoneidad",
             ylab = "Área (km^2)",
...
#        margin=T) + spplot(mps, fill = "transparent", col = "white")
levelplot(lima, at = breaks, col.regions = cols, main = "       IPSL-CM6A-LR RCP 8.5 (2081 - 2100)",
          margin=F) + spplot(mps, fill = "transparent", col = "white")
###resolucion en grados geograficos 
...
bp1<- barplot(a45, ylim=c(0,29490),
              #main = "Area (km^2)",
              xlab = "Idoneidad",
              ylab = "Área (km^2)",
              col = c( "gray93", "green4", "yellow", "red"), border = T, 
