...
##########IPSL 4.5 LIMA
#Cargar datos
map <- raster("D:/Maxent/AREA Y LIMA2/Vasconcellea_candicans_IPSL45.asc")
plot(map)
...
levelplot(cat.act, at = breaks, col.regions = cols, main = "Distribución futura IPSL45",
...
aact #aha
########BARPLOT
bp<- barplot(aact, ylim=c(0,1376773),
             #main = "Area (km^2)",
             xlab = "Idoneidad",
             ylab = "Área (km^2)",
...
#        margin=T) + spplot(mps, fill = "transparent", col = "white")
levelplot(lima, at = breaks, col.regions = cols, main = "       IPSL-CM6A-LR RCP 4.5 (2081 - 2100)",
          margin=F) + spplot(mps, fill = "transparent", col = "white")
###Resolución en grados geográficos 
...
bp1<- barplot(a45, ylim=c(0,32353),
              #main = "Area (km^2)",
              xlab = "Idoneidad",
              ylab = "Área (km^2)",
              col = c( "gray93", "green4", "yellow", "red"), border = T, 
